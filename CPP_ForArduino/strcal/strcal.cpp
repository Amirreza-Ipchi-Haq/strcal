...
bool strcal::isnum(String n){//Assign a function which checks if a string represents a number (standard (like 1) or non-standard (like 1.0))
	if(!n.length()||(n.length()<2&&!isWhole(n)))//Return false if the string is empty, or if it contains one character, it's not a digit
		return 0;
	...
	if(strExists(n,".")){//(A '.' exists)
		if(n[0]=='.'||n[n.length()-1]=='.'||n.indexOf('.')!=n.lastIndexOf('.'))//Return false if the first/last index of the string is '.', or there are more than one '.'
			return 0;
...
String strcal::fixnum(String n){//Assign a function which turns a number string into its standard form
	...
	if(strExists(n,".")){//(The string contains '.')
		while(n[n.length()-1]=='0'||n[n.length()-1]=='.')//Remove the '0's at the right
			if(n[n.length()-1]=='.'){//Terminate removal if the last character is '.'
				n.remove(n.length()-1);
				break;
...
String strcal::returnPoint(String n,size_t posFromRight){//Assign a function which returns '.' back to a string
...
void strcal::swapstr(String &a,String &b){
	String tmp=a;
	a=b,b=tmp;
	return;
}
void strcal::removeDecimals(String &x,String &y){//Assign a function which removes the floating point by multiplying both numbers by 10
	if(strExists(x,".")&&!strExists(y,"."))//Add '0's with the number of decimals in the first string to the second string and remove the '.' from the first one if the first string contains '.' and the second string doesn't
		...
		if(x.length()-x.indexOf('.')<y.length()-y.indexOf('.'))//Swap both strings and set the indicator to 1 (which represents true) if the second string has more decimals than the first string
			swapstr(x,y),swap=1;
...
uint8_t strcal::cmpstr(String a,String b){//Assign a function which compares strings (Same as `strcmp` whith the difference that it works with C++ style strings and the return type & value are different)
	if(a.length()>b.length())//Return 1 indicating that the first string is bigger if it has more characters than the second string
		return 1;
	if(a.length()<b.length())//Return 2 indicating the exact opposite if the condition above is exactly the opposite
		return 2;
...
uint8_t strcal::mod10(int8_t n){//Assign a function which finds mod10
...
String strcal::subtractWhole(String x,String y){//Assign a function which subtracts a whole number from another as strings
	...
	for(size_t i=x.length();i--;)//Do the subtraction digit by digit
		if(i<x.length()-y.length())
			answer=char(mod10(x[i]-'0'-subtract)+'0')+answer,subtract=x[i]-'0'-subtract<0?1:0;
		else
			answer=char(mod10(x[i]-y[i-(x.length()-y.length())]-subtract)+'0')+answer,subtract=x[i]-y[i-(x.length()-y.length())]-subtract<0?1:0;
	return fixnum(sign?"-"+answer:answer);//Return the answer in its standard form with the sign back
}
String strcal::simpleWholeDivide(String x,String y,bool option){//Assign a function which does division between 2 numbers as strings in a simple way (useful if x/y<10)
	...
	if(option&&(y=="2"||y=="5"))//Return the remainder of the last digit of the first string divided by the second string if the function asks for the remainder and the second string is either "2" or "5"
		return String(char((x[x.length()-1]-'0')%(y[0]-'0')+'0'));
...
String strcal::divideWhole(String x,String y,bool option){//(Same as the function above with speed improvements (Uses the function above))
	if(y=="0")//Return an empty string if the second string is "0"
		return "";
	if(y=="1"){//(The second string is "1")
		if(option)//Return "0" if the function asks for the remainder
	...
	for(size_t i=x.length();i--;answer0=answer0+simpleWholeDivide(answer1,y,0),answer1=fixnum(simpleWholeDivide(answer1,y,1)+(i?String(char(x[x.length()-i])):"")));//Do the division digit by digit
...
String strcal::gcd(String x,String y){//Assign a function which finds the greatest common divisor between 2 natural numbers as strings using the Euclidean algorithm
	...
	while(x[x.length()-1]<'1'&&y[y.length()-1]<'1')//Remove the common factor 10 from both strings
...
String strcal::calculate(String x,String y,char operation){//Assign the function which calculates 2 numbers with a given operator
	...
	switch(operation){//Check the operator
		...
		case '*'://Do the multiplication
				...
				for(size_t i=x.length();i--;answer=addWhole(answer,(add0?String(char(add0+'0')):"")+answer0),add0=0,answer0=mltstr("","0",x.length()-i))//Do the multiplication digit by digit
					for(size_t j=y.length();j--;answer0=char((add0+add1)%10+'0')+answer0,add0=(add0+add1)/10,add1=0)
		...
		case '/':
			...
			{//(Has local variables
				String divide=gcd(x,y);//Assign the division simplifier string
			...
			if(x!="0"){//(The answer has decimals)
				...
				for(;tmp[tmp.length()-1]<'1';tmp.remove(tmp.length()-1),x=x+"0",answer=answer+simpleWholeDivide(x,y,0),x=simpleWholeDivide(x,y,1))//Continue the division digit by digit and divide the temporary string by 10 until it's not divisible by 10
					if(x=="0")//Skip to the returning if the remainder is "0"
						goto finish;
				if(tmp[tmp.length()-1]=='5'){//(The temporary string is still divisible by 5)
					for(;tmp[tmp.length()-1]=='5';tmp=divideWhole(tmp,"5",0),x=x+"0",answer=answer+simpleWholeDivide(x,y,0),x=simpleWholeDivide(x,y,1))//Continue the division digit by digit and divide the temporary string by 5 until it's not divisible by 5
						if(x=="0")//Skip to the returning if the remainder is "0"
							goto finish;
				}else//(The temporary string is not divisible by 5)
					for(;(tmp[tmp.length()-1]-'0')%2<1;tmp=divideWhole(tmp,"2",0),x=x+"0",answer=answer+simpleWholeDivide(x,y,0),x=simpleWholeDivide(x,y,1))//Continue the division digit by digit and divide the temporary string by 2 until it's not divisible by 2
...
}//The rest are just for the `strnum` class methods
bool strcal::strnum::isWhole(){
	return strcal::isWhole(num);
}
bool strcal::strnum::strExists(String s){
	return strcal::strExists(num,s);
}
String strcal::strnum::rmstr(String s){
	num=strcal::rmstr(num,s);
	return num;
}
String strcal::strnum::absstr(){
	num=strcal::absstr(num);
	return num;
}
bool strcal::strnum::isnum(){
	return strcal::isnum(num);
}
String strcal::strnum::fixnum(){
	num=strcal::fixnum(num);
	return num;
}
String strcal::strnum::returnPoint(size_t posFromRight){
	num=strcal::returnPoint(num,posFromRight);
	return num;
}
void strcal::strnum::swapstr(String &s){
	strcal::swapstr(num,s);
	return;
}
uint8_t strcal::strnum::cmpstr(String s){
	return strcal::cmpstr(num,s);
}
String strcal::strnum::gcd(String n){
	return strcal::gcd(num,n);
}
String strcal::strnum::calculate(String n,char operation){
